...
struct RemovePatientattendsAppointmentScreen: View { 
	@State var x :String = ""
  	@State var y :String = ""
  ...
  var body: some View {
    ...
    VStack(spacing: 20) {
      ...
      HStack(spacing: 20) {
        Button(action: { self.model.removePatientattendsAppointment(x: x, y: y) } ) { Text("Remove") }
        Button(action: { self.model.cancelRemovePatientattendsAppointment() } ) { Text("Cancel") }
